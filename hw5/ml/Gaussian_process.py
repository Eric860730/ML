...
def rationalQuadraticKernel(
    ...
    parameter
    Xi: the array of all X coordinate
    Xj: the array of all X coordinate
...
def computeGaussianProcess(
    ...
    """
    parameter
    X and Y: Coordinate of input data points
...
def computeNegativeMarginalLogLikelihood(
        theta: np.ndarray,
        X: np.ndarray,
        Y: np.ndarray,
        beta: float):
    ...
    # negative log-likelihood -ln p(y|θ) = 0.5 * ln |Cθ| +0.5 * y^T * Cθ^-1 *
    # y +0.5 * N * ln(2π)
...
def visualizeGaussianProcess(
        X: np.ndarray,
        Y: np.ndarray,
        X_star: np.ndarray,
        predict_mean: np.ndarray,
        predict_variance: np.ndarray,
        opt_predict_mean: np.ndarray,
        opt_predict_variance: np.ndarray):
    ...
    drawRegression(X_star[:, 0], opt_predict_mean[:, 0],
                   opt_confidence_interval_95)
    plt.scatter(X, Y, s=10.0, color='black')
...
def gaussianProcessRegression(input_data: np.ndarray):
    """
    main function of Gaussian Process Regression.

    default
    β = 5.0, the variance of the noise ϵ.
    x_range = [-60, 60]
    num_slice = 1200
    ...
    # create the array of X_star, reshape(-1, 1) means reshape array to one
    # column and automatically compute the number of rows.
    num_slice = 1200
